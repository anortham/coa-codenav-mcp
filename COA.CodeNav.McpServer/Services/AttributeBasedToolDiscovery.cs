    ...
    public class AttributeBasedToolDiscovery
    {
        ...
        private object BuildSchemaFromType(Type type)
        {
            ...
            foreach (var prop in type.GetProperties(BindingFlags.Public | BindingFlags.Instance))
            {
                ...
                // Use JsonPropertyName if available, otherwise convert to camelCase
                var jsonPropertyName = prop.GetCustomAttribute<JsonPropertyNameAttribute>();
                var propName = jsonPropertyName?.Name ?? ConvertPropertyNameToJsonName(prop.Name);
                ...
                if (prop.PropertyType.IsValueType && Nullable.GetUnderlyingType(prop.PropertyType) == null)
                {
                    ...
                    var isRequired = prop.GetCustomAttribute<JsonRequiredAttribute>() != null ||
                                   prop.GetCustomAttributes().Any(a => a.GetType().Name == "RequiredAttribute");
                    
                    if (isRequired || type.GetProperties().Any(p => p.Name == prop.Name && p.GetCustomAttributes().Any(a => a.GetType().Name == "RequiredAttribute")))
                    {
                ...
                try
                {
                    ...
                    else if (methodParams.Length == 1 && IsComplexType(methodParams[0].ParameterType))
                    {
                        ...
                        if (args.HasValue)
                        {
                            ...
                            IsComplexType(methodParams[0].ParameterType) && 
                            methodParams[1].ParameterType == typeof(CancellationToken))
                    {
                        // Complex parameter followed by CancellationToken
                        if (args.HasValue)
                        {
                            var json = args.Value.GetRawText();
                            invokeArgs[0] = JsonSerializer.Deserialize(json, methodParams[0].ParameterType,
