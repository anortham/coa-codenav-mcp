using System.ComponentModel.DataAnnotations;
using System.Text.Json;
using System.Text.Json.Serialization;
using COA.CodeNav.McpServer.Constants;
using COA.CodeNav.McpServer.Infrastructure.TypeScript;
using COA.CodeNav.McpServer.Models;
using COA.CodeNav.McpServer.ResponseBuilders.TypeScript;
using COA.CodeNav.McpServer.Services;
using COA.CodeNav.McpServer.Services.TypeScript;
using COA.Mcp.Framework;
using COA.Mcp.Framework.Base;
using COA.Mcp.Framework.Models;
using COA.Mcp.Framework.TokenOptimization;
using COA.Mcp.Framework.TokenOptimization.Models;
using COA.Mcp.Framework.TokenOptimization.ResponseBuilders;
using Microsoft.Extensions.Logging;
using AIAction = COA.Mcp.Framework.Models.AIAction;
using ToolExecutionMetadata = COA.Mcp.Framework.Models.ToolExecutionMetadata;

namespace COA.CodeNav.McpServer.Tools.TypeScript;

/// <summary>
/// MCP tool that automatically adds missing import statements in TypeScript files using TSP
/// </summary>
public class TsAddMissingImportsTool : McpToolBase<TsAddMissingImportsParams, TsAddMissingImportsResult>, IDisposable
{
    private const int SAFETY_TOKEN_LIMIT = 8000;
    private readonly ILogger<TsAddMissingImportsTool> _logger;
    private readonly TypeScriptWorkspaceService _workspaceService;
    private readonly TypeScriptCompilerManager _compilerManager;
    private readonly ITokenEstimator _tokenEstimator;
    private readonly AnalysisResultResourceProvider? _resourceProvider;
    private readonly TsAddMissingImportsResponseBuilder? _responseBuilder;
    private readonly Dictionary<string, TsServerProtocolHandler> _serverHandlers = new();
    private readonly SemaphoreSlim _serverLock = new(1, 1);

    public override string Name => ToolNames.TsAddMissingImports;
    public override ToolCategory Category => ToolCategory.Refactoring;
    
    public override string Description => @"Automatically add missing import statements for TypeScript symbols.
Returns: Updated file content with added imports and change details.
Prerequisites: Call ts_load_tsconfig first to load the TypeScript project.
Error handling: Returns specific error codes with recovery steps if import addition fails.
Use cases: Fixing compilation errors, auto-importing dependencies, code completion.
Not for: Organizing existing imports (use ts_organize_imports), removing unused imports (different tool).";

    public TsAddMissingImportsTool(
        ILogger<TsAddMissingImportsTool> logger,
        TypeScriptWorkspaceService workspaceService,
        TypeScriptCompilerManager compilerManager,
        ITokenEstimator tokenEstimator,
        AnalysisResultResourceProvider? resourceProvider = null,
        TsAddMissingImportsResponseBuilder? responseBuilder = null)
        : base(logger)
    {
        _logger = logger;
        _workspaceService = workspaceService;
        _compilerManager = compilerManager;
        _tokenEstimator = tokenEstimator;
        _resourceProvider = resourceProvider;
        _responseBuilder = responseBuilder;
    }

    protected override async Task<TsAddMissingImportsResult> ExecuteInternalAsync(
        TsAddMissingImportsParams parameters,
        CancellationToken cancellationToken)
    {
        var startTime = DateTime.UtcNow;
        
        _logger.LogDebug("TsAddMissingImports request: File={File}, Preview={Preview}",
            parameters.FilePath, parameters.Preview);

        try
        {
            // Validate TypeScript availability
            var tsError = _compilerManager.ValidateTypeScriptAvailability();
            if (tsError != null)
            {
                return new TsAddMissingImportsResult
                {
                    Success = false,
                    Message = "TypeScript is not available",
                    Error = tsError
                };
            }

            // Find workspace for file
            var workspace = _workspaceService.FindWorkspaceForFile(parameters.FilePath);
            if (workspace == null)
            {
                return CreateWorkspaceNotLoadedResult(parameters.FilePath);
            }

            // Get or create server handler
            var handler = await GetOrCreateServerHandlerAsync(
                workspace.WorkspaceId, 
                workspace.ProjectPath, 
                cancellationToken);
                
            if (handler == null)
            {
                return CreateErrorResult("Failed to start TypeScript language server",
                    new ErrorInfo { Code = ErrorCodes.INTERNAL_ERROR, Message = "Could not initialize TypeScript language server" });
            }

            // Get current file content
            var originalContent = await File.ReadAllTextAsync(parameters.FilePath, cancellationToken);

            // First, get semantic diagnostics to identify missing import errors
            var diagnostics = await handler.GetSemanticDiagnosticsAsync(parameters.FilePath, cancellationToken);
            if (diagnostics == null)
            {
                return new TsAddMissingImportsResult
                {
                    Success = true,
                    Message = "No missing imports detected - file appears to be error-free",
                    Query = new AddMissingImportsQuery
                    {
                        FilePath = parameters.FilePath,
                        Preview = parameters.Preview
                    },
                    Summary = new AddMissingImportsSummary
                    {
                        TotalImportsAdded = 0,
                        ErrorsFixed = 0
                    },
                    Meta = new ToolExecutionMetadata
                    {
                        ExecutionTime = $"{(DateTime.UtcNow - startTime).TotalMilliseconds:F2}ms"
                    }
                };
            }

            // Parse diagnostics to find missing import errors
            var missingImportErrors = ParseMissingImportErrors(diagnostics.Value);
            
            if (missingImportErrors.Count == 0)
            {
                return new TsAddMissingImportsResult
                {
                    Success = true,
                    Message = "No missing imports detected in this file",
                    Query = new AddMissingImportsQuery
                    {
                        FilePath = parameters.FilePath,
                        Preview = parameters.Preview
                    },
                    Summary = new AddMissingImportsSummary
                    {
                        TotalImportsAdded = 0,
                        ErrorsFixed = 0
                    },
                    Meta = new ToolExecutionMetadata
                    {
                        ExecutionTime = $"{(DateTime.UtcNow - startTime).TotalMilliseconds:F2}ms"
                    }
                };
            }

            // Get code fixes for each missing import error
            var allChanges = new List<TsFileChange>();
            var processedErrors = 0;

            foreach (var error in missingImportErrors)
            {
                try
                {
                    var codeFixes = await handler.GetCodeFixesAsync(
                        parameters.FilePath,
                        error.Line,
                        error.Character,
                        error.EndLine,
                        error.EndCharacter,
                        new[] { error.Code.ToString() },
                        cancellationToken);

                    if (codeFixes != null)
                    {
                        var changes = ParseCodeFixesResponse(codeFixes.Value, "Add missing import");
                        allChanges.AddRange(changes);
                        processedErrors++;
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Failed to get code fixes for error at line {Line}", error.Line);
                }
            }

            if (allChanges.Count == 0)
            {
                return new TsAddMissingImportsResult
                {
                    Success = false,
                    Message = "No automatic imports available for the missing symbols",
                    Query = new AddMissingImportsQuery
                    {
                        FilePath = parameters.FilePath,
                        Preview = parameters.Preview
                    },
                    Error = new ErrorInfo
                    {
                        Code = ErrorCodes.ADD_IMPORTS_FAILED,
                        Message = "TypeScript server could not provide automatic imports for the missing symbols",
                        Recovery = new RecoveryInfo
                        {
                            Steps = new[]
                            {
                                "Check if the required dependencies are installed",
                                "Verify the symbols exist in the available modules",
                                "Add imports manually if they are from custom modules"
                            }
                        }
                    },
                    Insights = new List<string>
                    {
                        $"Found {missingImportErrors.Count} missing import error(s)",
                        "Some symbols may need to be imported manually",
                        "Check package.json for missing dependencies"
                    }
                };
            }

            var updatedContent = ApplyChanges(originalContent, allChanges);

            // If not preview mode, write the changes to disk
            if (!parameters.Preview)
            {
                await File.WriteAllTextAsync(parameters.FilePath, updatedContent, cancellationToken);
                
                // Update the TypeScript server with the new content
                await handler.UpdateFileAsync(parameters.FilePath, updatedContent, cancellationToken);
            }

            var executionTime = (DateTime.UtcNow - startTime).TotalMilliseconds;

            // Build the result
            var result = new TsAddMissingImportsResult
            {
                Success = true,
                Message = parameters.Preview 
                    ? $"Preview: Add {allChanges.Count} missing import(s) to '{Path.GetFileName(parameters.FilePath)}'"
                    : $"Added {allChanges.Count} missing import(s) to '{Path.GetFileName(parameters.FilePath)}'",
                Query = new AddMissingImportsQuery
                {
                    FilePath = parameters.FilePath,
                    Preview = parameters.Preview
                },
                Summary = new AddMissingImportsSummary
                {
                    TotalImportsAdded = allChanges.Count,
                    ErrorsFixed = processedErrors
                },
                Changes = allChanges,
                OriginalContent = parameters.Preview ? originalContent : null,
                UpdatedContent = parameters.Preview ? updatedContent : null,
                MissingImports = missingImportErrors.Select(e => new MissingImportInfo
                {
                    Symbol = e.Symbol,
                    Line = e.Line,
                    Character = e.Character,
                    Message = e.Message
                }).ToList(),
                Insights = GenerateInsights(allChanges, missingImportErrors),
                Actions = GenerateActions(parameters.FilePath, allChanges.Count)
            };

            // Apply response builder optimization if available
            if (_responseBuilder != null)
            {
                var context = new ResponseContext
                {
                    ResponseMode = "optimized",
                    TokenLimit = SAFETY_TOKEN_LIMIT,
                    ToolName = Name
                };
                
                result = await _responseBuilder.BuildResponseAsync(result, context);
            }
            else
            {
                // Fallback to direct token optimization
                result = await ApplyTokenOptimization(result, executionTime);
            }

            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error adding missing imports for file: {FilePath}", parameters.FilePath);
            return CreateErrorResult($"Failed to add missing imports: {ex.Message}",
                new ErrorInfo { Code = ErrorCodes.INTERNAL_ERROR, Message = ex.Message });
        }
    }

    #region Helper Methods

    private async Task<TsServerProtocolHandler?> GetOrCreateServerHandlerAsync(string workspaceId, string projectPath, CancellationToken cancellationToken)
    {
        await _serverLock.WaitAsync(cancellationToken);
        try
        {
            if (_serverHandlers.TryGetValue(workspaceId, out var existingHandler) && existingHandler.IsRunning)
            {
                return existingHandler;
            }

            // Create a logger for TsServerProtocolHandler
            var handlerLogger = _logger as ILogger<TsServerProtocolHandler> 
                ?? new LoggerFactory().CreateLogger<TsServerProtocolHandler>();
            var handler = await TsServerProtocolHandler.CreateAsync(
                handlerLogger,
                projectPath,
                cancellationToken);
            if (handler != null)
            {
                _serverHandlers[workspaceId] = handler;
            }

            return handler;
        }
        finally
        {
            _serverLock.Release();
        }
    }

    private TsAddMissingImportsResult CreateWorkspaceNotLoadedResult(string filePath)
    {
        return new TsAddMissingImportsResult
        {
            Success = false,
            Message = $"No TypeScript project loaded for file: {filePath}",
            Error = new ErrorInfo
            {
                Code = ErrorCodes.WORKSPACE_NOT_LOADED,
                Message = "TypeScript project not loaded for this file",
                Recovery = new RecoveryInfo
                {
                    Steps = new[]
                    {
                        "Load the TypeScript project containing this file",
                        "Use ts_load_tsconfig to load the project's tsconfig.json",
                        "Ensure the file is included in the TypeScript project configuration"
                    },
                    SuggestedActions = new List<SuggestedAction>
                    {
                        new()
                        {
                            Tool = ToolNames.TsLoadTsConfig,
                            Description = "Load the TypeScript project",
                            Parameters = new Dictionary<string, object>
                            {
                                ["tsConfigPath"] = "path/to/tsconfig.json"
                            }
                        }
                    }
                }
            }
        };
    }

    private TsAddMissingImportsResult CreateErrorResult(string message, ErrorInfo error)
    {
        return new TsAddMissingImportsResult
        {
            Success = false,
            Message = message,
            Error = error
        };
    }

    private List<MissingImportError> ParseMissingImportErrors(JsonElement diagnostics)
    {
        var errors = new List<MissingImportError>();

        try
        {
            if (diagnostics.ValueKind == JsonValueKind.Array)
            {
                foreach (var diagnostic in diagnostics.EnumerateArray())
                {
                    if (diagnostic.TryGetProperty("code", out var code) &&
                        diagnostic.TryGetProperty("start", out var start) &&
                        diagnostic.TryGetProperty("messageText", out var messageText))
                    {
                        // Common TypeScript error codes for missing imports
                        var errorCode = code.GetInt32();
                        if (errorCode == 2304 || // Cannot find name
                            errorCode == 2339 || // Property does not exist on type
                            errorCode == 2307)   // Cannot find module
                        {
                            if (start.TryGetProperty("line", out var line) &&
                                start.TryGetProperty("character", out var character))
                            {
                                var symbol = ExtractSymbolFromMessage(messageText.GetString() ?? "");
                                if (!string.IsNullOrEmpty(symbol))
                                {
                                    errors.Add(new MissingImportError
                                    {
                                        Code = errorCode,
                                        Symbol = symbol,
                                        Line = line.GetInt32() + 1, // Convert to 1-based
                                        Character = character.GetInt32() + 1, // Convert to 1-based
                                        EndLine = line.GetInt32() + 1,
                                        EndCharacter = character.GetInt32() + symbol.Length + 1,
                                        Message = messageText.GetString() ?? ""
                                    });
                                }
                            }
                        }
                    }
                }
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to parse missing import errors");
        }

        return errors;
    }

    private string ExtractSymbolFromMessage(string message)
    {
        // Extract symbol name from TypeScript error messages
        // Examples: "Cannot find name 'React'", "Property 'useState' does not exist on type"
        
        if (message.StartsWith("Cannot find name "))
        {
            var startIndex = message.IndexOf('\'');
            var endIndex = message.IndexOf('\'', startIndex + 1);
            if (startIndex >= 0 && endIndex > startIndex)
            {
                return message.Substring(startIndex + 1, endIndex - startIndex - 1);
            }
        }
        else if (message.Contains("Property ") && message.Contains(" does not exist"))
        {
            var startIndex = message.IndexOf('\'');
            var endIndex = message.IndexOf('\'', startIndex + 1);
            if (startIndex >= 0 && endIndex > startIndex)
            {
                return message.Substring(startIndex + 1, endIndex - startIndex - 1);
            }
        }

        return "";
    }

    private List<TsFileChange> ParseCodeFixesResponse(JsonElement codeFixes, string filterDescription)
    {
        var changes = new List<TsFileChange>();

        try
        {
            if (codeFixes.ValueKind == JsonValueKind.Array)
            {
                foreach (var fix in codeFixes.EnumerateArray())
                {
                    if (fix.TryGetProperty("description", out var description) &&
                        description.GetString()?.Contains("Add import") == true &&
                        fix.TryGetProperty("changes", out var fixChanges) &&
                        fixChanges.ValueKind == JsonValueKind.Array)
                    {
                        foreach (var change in fixChanges.EnumerateArray())
                        {
                            if (change.TryGetProperty("textChanges", out var textChanges) &&
                                textChanges.ValueKind == JsonValueKind.Array)
                            {
                                foreach (var textChange in textChanges.EnumerateArray())
                                {
                                    var fileChange = ParseTextChange(textChange);
                                    if (fileChange != null)
                                    {
                                        fileChange.Description = description.GetString() ?? "Add missing import";
                                        changes.Add(fileChange);
                                    }
                                }
                            }
                        }
                        break; // Use the first applicable fix
                    }
                }
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Failed to parse code fixes response");
        }

        return changes;
    }

    private TsFileChange? ParseTextChange(JsonElement textChange)
    {
        try
        {
            if (!textChange.TryGetProperty("span", out var span) ||
                !textChange.TryGetProperty("newText", out var newText))
                return null;

            if (!span.TryGetProperty("start", out var start) ||
                !span.TryGetProperty("length", out var length))
                return null;

            return new TsFileChange
            {
                StartPosition = start.GetInt32(),
                Length = length.GetInt32(),
                NewText = newText.GetString() ?? "",
                Description = "Add missing import"
            };
        }
        catch
        {
            return null;
        }
    }

    private string ApplyChanges(string originalContent, List<TsFileChange> changes)
    {
        // Sort changes by position descending to apply from end to start
        var sortedChanges = changes.OrderByDescending(c => c.StartPosition).ToList();
        
        var content = originalContent;
        foreach (var change in sortedChanges)
        {
            if (change.StartPosition >= 0 && change.StartPosition <= content.Length)
            {
                var endPos = Math.Min(change.StartPosition + change.Length, content.Length);
                content = content.Substring(0, change.StartPosition) + 
                         change.NewText + 
                         content.Substring(endPos);
            }
        }

        return content;
    }

    private List<string> GenerateInsights(List<TsFileChange> changes, List<MissingImportError> errors)
    {
        var insights = new List<string>();

        if (changes.Count > 0)
        {
            insights.Add($"Successfully added {changes.Count} missing import(s)");
            
            var importsAdded = changes.Where(c => c.NewText.StartsWith("import ")).Count();
            if (importsAdded > 0)
            {
                insights.Add($"Added {importsAdded} new import statement(s)");
            }
        }

        if (errors.Count > changes.Count)
        {
            insights.Add($"{errors.Count - changes.Count} error(s) may require manual import resolution");
        }

        insights.Add("Run ts_organize_imports to organize the newly added imports");
        insights.Add("Check ts_get_diagnostics to verify all import errors are resolved");

        return insights;
    }

    private List<AIAction> GenerateActions(string filePath, int changeCount)
    {
        var actions = new List<AIAction>();

        if (changeCount > 0)
        {
            actions.Add(new AIAction
            {
                Action = ToolNames.TsOrganizeImports,
                Description = "Organize the newly added imports",
                Parameters = new Dictionary<string, object>
                {
                    ["filePath"] = filePath
                }
            });

            actions.Add(new AIAction
            {
                Action = ToolNames.TsGetDiagnostics,
                Description = "Check for remaining TypeScript errors",
                Parameters = new Dictionary<string, object>
                {
                    ["filePath"] = filePath
                }
            });
        }

        return actions;
    }

    private Task<TsAddMissingImportsResult> ApplyTokenOptimization(TsAddMissingImportsResult result, double executionTime)
    {
        // Estimate current token usage
        var estimatedTokens = _tokenEstimator.EstimateObject(result);
        var wasOptimized = false;
        
        if (estimatedTokens > SAFETY_TOKEN_LIMIT)
        {
            _logger.LogDebug("TsAddMissingImports: Token optimization needed. Estimated: {EstimatedTokens}, Limit: {Limit}", 
                estimatedTokens, SAFETY_TOKEN_LIMIT);
            
            // Progressive reduction strategy
            if (result.Changes?.Count > 20)
            {
                // Keep first 20 most important changes (imports are prioritized by TypeScript server)
                result.Changes = result.Changes.Take(20).ToList();
                result.Insights?.Add($"⚠️ Showing first 20 of {result.Summary?.TotalImportsAdded} changes due to size limits");
                wasOptimized = true;
            }
            
            // Reduce content if still over limit and in preview mode
            if (_tokenEstimator.EstimateObject(result) > SAFETY_TOKEN_LIMIT && result.OriginalContent != null)
            {
                if (result.OriginalContent.Length > 2000)
                {
                    result.OriginalContent = result.OriginalContent[..2000] + "\n... (truncated)";
                }
                if (result.UpdatedContent != null && result.UpdatedContent.Length > 2000)
                {
                    result.UpdatedContent = result.UpdatedContent[..2000] + "\n... (truncated)";
                }
                wasOptimized = true;
            }
            
            // Store full result in resource provider if available
            if (wasOptimized && _resourceProvider != null)
            {
                try
                {
                    var resourceUri = _resourceProvider.StoreAnalysisResult("ts-add-missing-imports", result);
                    result.ResourceUri = resourceUri;
                    result.Insights?.Add("💾 Complete results stored - use resource tools to access full data");
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Failed to store full result in resource provider");
                }
            }
        }
        
        // Update metadata
        result.Meta = new ToolExecutionMetadata
        {
            Mode = wasOptimized ? "optimized" : "full",
            Truncated = wasOptimized,
            Tokens = _tokenEstimator.EstimateObject(result),
            ExecutionTime = $"{executionTime:F2}ms"
        };
        
        return Task.FromResult(result);
    }

    #endregion

    public void Dispose()
    {
        foreach (var handler in _serverHandlers.Values)
        {
            try
            {
                handler?.Dispose();
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Error disposing TypeScript server handler");
            }
        }
        _serverHandlers.Clear();
        _serverLock?.Dispose();
    }
}

/// <summary>
/// Parameters for adding missing imports in a TypeScript file
/// </summary>
public class TsAddMissingImportsParams
{
    [Required]
    [JsonPropertyName("filePath")]
    public required string FilePath { get; set; }

    [JsonPropertyName("preview")]
    public bool Preview { get; set; } = false;
}
